...
SELECT a.customer_name,a.region FROM (SELECT * FROM customers WHERE region='South') a;
...
FROM dept_manager dm,(SELECT dept_no,dept_name FROM departments)d,(SELECT first_name,emp_no,last_name FROM employees)e
...
(SELECT customer_name FROM customers c WHERE s.customer_id=c.customer_id)
FROM sales s
ORDER BY customer_id;
...
SELECT e.emp_no, b.dept_no, e.first_name, e.last_name
FROM employees e
JOIN
(SELECT* FROM dept_emp 
WHERE dept_no IN (SELECT dept_no FROM departments 
...
-- Solution
SELECT * FROM dept_manager
WHERE from_date > '1985-01-01' AND dept_no IN
...
SELECT emp_no, ROUND(AVG(salary),2) avg_salary 
FROM salaries
WHERE salary > 120000
AND emp_no IN (SELECT emp_no FROM dept_emp
              WHERE dept_no IN ('d002','d003'))
...
-- Solution
SELECT c.customer_id,c.customer_name,s.cust_count 
FROM customers c,
(SELECT customer_id, COUNT(*) AS cust_count
FROM sales
GROUP BY customer_id
...
-- Solution
SELECT c.customer_id,c.customer_name,c.segment,s.cust_count,s.total_sales
FROM customers c,
(SELECT customer_id, COUNT(*) AS cust_count, SUM(sales) total_sales
FROM sales
GROUP BY customer_id
ORDER BY total_sales DESC, cust_count DESC) s
WHERE c.customer_id=s.customer_id
ORDER BY s.total_sales DESC,s.cust_count DESC;
